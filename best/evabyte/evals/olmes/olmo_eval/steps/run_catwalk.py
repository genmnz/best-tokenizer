...
from catwalk.steps_simple import get_instances
...
class ConstructCatwalkModel(Step):
    ...
    def run(
        ...
        return DecoderOnlyLanguageModel(hf_name, **model_args)
...
class PredictAndCalculateMetricsStep(Step):
    ...
    def run(
        ...
        **kwargs,
        ...
        predictions = [
            result
            for result in model_obj.predict(
                task,
                instances,
        ...
        metrics = model_obj.calculate_metrics(
            task, predictions
        ...
        model_kwargs = {}
        if hasattr(model_obj, "model_kwargs"):
            model_kwargs.update(model_obj.model_kwargs)
        output = {
            "task": task_dict["name"],
