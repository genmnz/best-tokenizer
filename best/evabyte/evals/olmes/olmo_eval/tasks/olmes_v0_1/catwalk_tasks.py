...
TASKS_STD: Dict[str, Task] = {
    ...
    "hellaswag_zeroshot_full": EleutherMMLUTask("hellaswag_zeroshot_full", ranked_classification=True).add_metrics(
        rc_metrics(primary="acc_per_char")
    ),
    "hellaswag_mc_zeroshot_full": EleutherMMLUTask(
        "hellaswag_mc_zeroshot_full", ranked_classification=True
    ).add_metrics(rc_metrics(primary="acc_raw")),
    **create_catwalk_mmlu_std_tasks(),
    ...
    "winogrande_zeroshot": EleutherMMLUTask("winogrande_zeroshot", ranked_classification=True).add_metrics(
        rc_metrics(primary="acc_raw")
    ),
    "winogrande_mc_zeroshot": EleutherMMLUTask(
        "winogrande_mc_zeroshot", ranked_classification=True
    ).add_metrics(rc_metrics(primary="acc_raw")),
