...
def combine_metrics(tasks):
    ...
    for task in tasks:
        ...
        all_metrics = {
            k: v for k, v in task["metrics"]["rc_metrics"].items()
            if "acc_" in k
        }
        all_metrics["acc_best"] = max(all_metrics.values())
        ...
        for group, regex in GROUP_AGGREGATORS.items():
            if re.match(regex, task_name):
                ...
                grouped_task_scores[group] = grouped_task_scores.get(group, []) + [
                    {"score": task_score, "num_instances": num_instances, "all_metrics": all_metrics}
                ]
                break
        if not group_found:
            task_scores.append(
                {"task": task_name, "score": task_score, "num_instances": num_instances, "all_metrics": all_metrics}
            )
    for group, scores in grouped_task_scores.items():
        ...
        all_metrics_avg = {
            metric: sum(x["all_metrics"][metric] for x in scores) / len(scores)
            for metric in scores[0]["all_metrics"]
        }
        del all_metrics_avg["acc_best"]
        all_metrics_avg["acc_best"] = max(all_metrics_avg.values())
        task_scores.append(
            {
                "task": group,
                "score": macro_avg,
    ...
    for task_score in task_scores:
        ...
        combined_rc_mc[key] = combined_rc_mc.get(key, []) + [
            {"mc": is_mc, "score": task_score["score"], "all_metrics": task_score["all_metrics"]}
        ]
    results = []
    for task, scores in combined_rc_mc.items():
        ...
        result = {
            ...
            "score": winner["score"],
            "used_mc": winner["mc"],
            "all_metrics": winner["all_metrics"],
            "all_scores": scores,
        }
