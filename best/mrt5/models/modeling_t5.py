...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        #### NEW CODE ####
        self.has_absolute_position_embeddings = config.has_absolute_position_embeddings
        ...
        #### NEW CODE ####
        # Include a check for absolute position embeddings
        if self.has_relative_attention_bias and not self.has_absolute_position_embeddings:
        #### NEW CODE ####
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module, initializer_factor=None):
        """Initialize the weights"""
        #### NEW CODE ####
        factor = self.config.initializer_factor if initializer_factor is None else initializer_factor
        #### NEW CODE ####
        if isinstance(module, T5LayerNorm):
            #### NEW CODE ####
            # Remove the factor multiplier
            module.weight.data.fill_(factor * 1.0)
            #### NEW CODE ####
        elif isinstance(
            module,
            (T5Model, T5ForConditionalGeneration,
            T5EncoderModel, T5ForQuestionAnswering),
        ):
            # Mesh TensorFlow embeddings initialization
            # See https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/layers.py#L1624
            #### NEW CODE ####
            # Set embeddings std to 1/sqrt(d_model) instead of factor * 1.0
            module.shared.weight.data.normal_(
                mean=0.0, std=((self.config.d_model) ** -0.5))
            #### NEW CODE ####
            if hasattr(module, "lm_head") and not self.config.tie_word_embeddings:
                #### NEW CODE ####
                # Set lm_head std to 1/sqrt(d_model) instead of factor * 1.0
        ...
        #### NEW CODE ####
        # Add absolute position embeddings initialization
        elif isinstance(module, T5Stack) and module.config.has_absolute_position_embeddings:
            module.absolute_pos_embed.weight.data.normal_(
                mean=0.0, std=((self.config.d_model) ** -0.5))
        #### NEW CODE ####
            ...
            #### NEW CODE ####
            # Include a check for absolute position encodings
            if module.has_relative_attention_bias and not module.has_absolute_position_embeddings:
            #### NEW CODE ####
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.absolute_pos_embed = \
            nn.Embedding(config.max_position_embeddings, self.config.d_model) \
            if config.has_absolute_position_embeddings else None
        #### NEW CODE ####
